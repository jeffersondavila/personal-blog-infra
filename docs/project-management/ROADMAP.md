...
- **Última actualización:** 2026-09-14 — `Task/025-Terraform-Cloud` **Aprobada** mediante `approved: Task/025-Terraform-Cloud`. El avance pasa a **25/41 ≈ 61 %** y la **ETAPA 08** queda **En progreso** con **3 de 4 — 75 %**: **no está completada**, `Task/026` sigue **Pendiente, no iniciada**. Entrega Terraform **portable** con un **solo grafo de 21 recursos** —provider oficial `hashicorp/aws = 6.64.0`, CLI `= 1.16.2`, lock **versionado** para `linux_amd64` y `windows_amd64`— y lo **ejecuta de verdad** contra el laboratorio AWS local: `apply`, pruebas funcionales de S3, SSM, IAM, Lambda y Logs, idempotencia, `destroy` con **ausencia verificada por API**, **reconstrucción**, *smoke* y segundo `destroy`. **Camino crítico demostrado** (R-25): `GET /health` por HTTP real API Gateway v2 → Lambda → *handler* devolvió **200** con el **mismo cuerpo** que el entorno local. Matriz de paridad con evidencia real y **sin ninguna celda de «paridad completa»**. **D-06 RESUELTA**: estado `local` fuera del árbol de Git y fuera del emulador; para AWS real, `s3` con `use_lockfile = true`, **sin DynamoDB**, *bootstrap* aparte — **el bucket no existe**. **Cero recursos AWS reales.** La aprobación **incluye dos excepciones humanas**: **H-025-1** (el criterio **7** **no se cumple literalmente**: **11 PASS literal + 1 PASS con excepción humana**, ningún FAIL) y la **temporal de S-09** de **79 identidades** (**H-025-6**, **H-025-7**), **sin** declararse resueltas. La revisión final del diff encontró y corrigió **dos defectos propios** que habrían puesto el CI en rojo: **DEF-025-1** y **DEF-025-2**. **D-11**, **D-12** y **H-023-3** siguen abiertos. Corregidas **D-025-1**, **D-025-2** y **C-025-1**.
...
- **Avance global:** **≈ 61 %** (25 de 41 tareas aprobadas)
...
| 08 | Preparación Cloud + AWS Local Parity | 4 | **2** | **50 %** | **En progreso** | 07 ✔ |
