...
**Última actualización:** 2026-09-21 — **addendum Floci de
...
**Fase ejecutada el 2026-09-21, tras la aprobación:** la
[estrategia de cierre por fases](../task-reports/TASK-027.1-report.md#15-reconciliación-documental-y-estrategia-de-cierre--2026-09-21)
autoriza en este punto registrar la aprobación, **crear los commits del addendum sobre
`df3ba33` y publicar únicamente la rama `Task/027.1`**. Ahí termina: **sin PR, sin
integración en `dev` y sin cambiar a `Task/027`**. La consolidación en 027 requiere otra
autorización; habrá un único PR nuevo `Task/027 → main`. No reabrir #47/#48 ni crear otro
PR de 027.1. El SHA remoto resultante consta en el
...
| Campo | Valor |
| --- | --- |
| **Tarea actual** | **Task/027.1-Corregir-Regresion-S09-MinIO: Aprobada.** El usuario aprobó el **addendum Floci** el **2026-09-21** mediante `approved: Task/027.1-Corregir-Regresion-S09-MinIO`; sus decisiones pasan a **Aceptadas y Vigentes**, sin ADR nuevo. Primera aprobación MinIO del 2026-09-20 conservada como historia. **Es mantenimiento: no cuenta entre las 41 ni altera el avance.** El cierre se detiene tras publicar la rama: sin PR ni integración en `dev`. |
| **Etapa actual** | **ETAPA 09 — Cuentas y Seguridad Cloud — En progreso, 1/3 ≈ 33 %.** Task/027 aprobada el 2026-09-17; Task/028 y Task/029 pendientes. ETAPAS 00 a 08 completadas; ETAPA 08 **4/4, 100 %** desde Task/026. La evidencia de Floci sigue siendo local. |
...
| **Última tarea canónica aprobada** | **`Task/027-Configurar-Cuentas-y-Presupuestos` — Aprobada** el 2026-09-17. **27/41 ≈ 66 %; ETAPA 09 1/3 ≈ 33 %.** Alcance aprobado: cuentas AWS/Cloudflare, IAM administrativo separado, MFA, cero access keys, Free Plan, presupuesto y alertas; **D-13 resuelta**. Evidencia original identificada en el reporte de Task/027.1 §15; su integración requiere autorización posterior. |
...
| **Tarea en curso** | **Ninguna.** `Task/027.1` quedó **Aprobada** el 2026-09-21 y publicada; es mantenimiento fuera de las 41. La consolidación en `Task/027` espera otra autorización del usuario. |
| **Próxima tarea prevista** | `Task/028-GitHub-OIDC-AWS` — **Pendiente, no iniciada**. Aprobar el addendum 027.1 **no** autoriza iniciar Task/028. Toda nueva Task nace de `main` actualizado y limpio según [WORKFLOW §2.1](WORKFLOW.md). |
...
| **Decisiones abiertas** | **12** — **D-13 resuelta** por Task/027 el 2026-09-17. D-21 y las demás decisiones diferidas conservan sus propietarios. La reconciliación del entregable original de Task/027 se hará en la consolidación autorizada. |

> **Recuento tras la aprobación de `Task/018`, 2026-09-08:**
> **18 de 41 — 44 %**; ETAPA 05 **Completada, 3 de 3 — 100 %**.
> **E-06 cerrado** por HTTP; **D-21** sigue abierta y **ADR-009** en Propuesta.
>
> **Recuento histórico tras la aprobación de `Task/017`, 2026-09-06:** el avance pasa a
...
>
> **Resultado histórico de `Task/016`, aprobada el 2026-09-06:** **E-03** quedó **no
> cerrado** y **E-06 parcial**, ambos con su motivo **medido** en cuatro canales.
...
pasa de **22/41 ≈ 54 %** a **23/41 ≈ 56 %**. Abre la **ETAPA 08**, que queda **En progreso
con 1 de 4 tareas aprobadas (25 %)**. **La etapa no está completada.**

**Qué entrega.** El adaptador que permite ejecutar **la misma** aplicación FastAPI bajo AWS
Lambda con eventos de **API Gateway HTTP API v2** (*payload format* 2.0):
`personal-blog-backend/app/lambda_handler.py`, **un archivo** más una línea de
`pyproject.toml`. `app/main.py`, el `Dockerfile` y el Compose **no cambian**, y
`uvicorn app.main:app` sigue siendo el entrypoint local. **Una dependencia nueva
autorizada**, `mangum==0.22.0`, sin transitivas nuevas y sin mover `--exclude-newer`.

**Qué demostró.** *Observado el 2026-09-13:* **42 pruebas nuevas** —25 de contrato, 11
unitarias, 6 de integración— y suite completa en **1918 passed, 1 skipped** contra
PostgreSQL y MinIO reales, con el único *skip* estructural de Windows. El **flujo
administrativo auténtico** recorrido entero a través del *handler*: `login` → cookie en
`cookies[]` → `/me` en `200`, contra PostgreSQL real. `pip-audit --strict` sin
vulnerabilidades en ambos *locks*; imagen Docker construida con `--require-hashes`; y el
*handler* respondiendo `200` **dentro de la imagen Linux** con `-W error`.

**`lifespan="off"` no se eligió por intuición.** Se descargó la distribución de la versión
**fijada**, se comprobó su `sha256` contra PyPI —el mismo que quedó en los *locks*— y se
leyó su código: el ciclo de vida se ejecutaría **una vez por invocación**, y `"auto"` no
degradaría porque Starlette **sí** implementa el protocolo. La premisa que sostiene la
decisión queda protegida por **tres guardas independientes**.

**Lo que destapó.** Tres defectos del propio entregable, corregidos: una guarda inicial que
medía un detalle interno de FastAPI en lugar de la premisa (**H-023-0**); un
`DeprecationWarning` de `asyncio.get_event_loop()` en Python 3.12 que `-W error` convertía
en error, resuelto **sin silenciarlo** y con regresión permanente (**H-023-1**); y un
desajuste de tipos que `mypy --strict` destapó, resuelto **sin `type: ignore`**
(**H-023-2**).

**Lo que NO cierra.** **H-023-3** sigue **ABIERTO y NO DIAGNOSTICADO**: una ejecución
completa produjo un fallo en
`test_dos_publicaciones_simultaneas_solo_prosperan_una`, cuya traza no se conservó y que
**no se reprodujo** en ocho intentos posteriores. **La aprobación no lo convierte en
resuelto**, no se atribuye al adaptador —que no participa en el camino de código de ese
test— y **debe vigilarse en la CI**. **P-07** no se toca: es de `Task/024` y `Task/032`.

[Ficha](../tasks/TASK-023-fastapi-lambda-compatibility.md) ·
[Reporte](../task-reports/TASK-023-report.md).

## Tarea aprobada anterior — `Task/022-Validacion-Local-Production-Like`

**Estado: Aprobada** el **2026-09-12** mediante
`approved: Task/022-Validacion-Local-Production-Like`. **Cuenta en las 41 tareas**: el
avance pasa de **21/41 ≈ 51 %** a **22/41 ≈ 54 %**, y la **ETAPA 07 queda Completada**
con **1 de 1 tarea aprobada**.

**Qué entrega.** La **semilla local** (`personal-blog-backend/scripts/seed_local.py`): el
`Administrator` y el `Profile` que la base local **nunca había tenido**, porque `Task/012`
decidió (**D-012-U**) que el perfil no se crea por API. Escrita test-first, con matriz de
12 casos, RED demostrado y **21 pruebas nuevas** tras H-8 (20 previas y una regresión adicional de invocación). Crea el mínimo imprescindible: el
contenido de prueba lo generó el **recorrido administrativo real**.

**Qué demostró.** Los **ocho criterios de salida** de STAGE-07. *Observado el 2026-09-12:*
reconstrucción desde cero con **17 tablas** sobre base vacía; los cuatro verbos —crear,
editar, publicar, archivar— sobre los cuatro tipos publicables; imagen servida por enlace
firmado con los **mismos 4836 bytes**; persistencia intacta tras reiniciar sin tocar
volúmenes; restauración con **conteos iguales y huella `md5` idéntica fila a fila**; y una
**recuperación total real** que dejó el entorno **operativo**, con login administrativo
`200` y `/ready` en `200`. **T-07 satisfecho.** Portainer y el recorrido visual los
**confirmó el usuario**.

**Lo que destapó, que es el valor menos esperado.** **Siete defectos de los runbooks**, y
ninguno se habría visto sin reconstruir de verdad: el procedimiento no construía las
imágenes (**H-1**), confundía qué imágenes son de terceros (**H-6**), no creaba el bucket
de medios —lo que hacía terminar la provisión en **exit 1**— (**H-2**), no advertía de que
`down -v` destruye la base de pruebas (**H-4**), copiaba a `/tmp`, imposible desde el
endurecimiento de `Task/018` (**H-7**), y **la recuperación total devolvía los datos
dejando la aplicación sin permisos, en silencio** (**H-5**). Causa común: `Task/010` y
`Task/018` cambiaron el entorno y **nadie actualizó los procedimientos**. **H-3** fue un
defecto del propio entregable, corregido con regresión permanente. Los siete están
corregidos, y H-5 **revalidado punto por punto** con una recuperación real.

**H-8 — Resuelto durante la integración aprobada.** *Observado el 2026-09-13 UTC:*
CI Backend `34729289407` detectó que la invocación de la semilla dependía del `.env`
local: construía `Settings` antes de validar la entrada. El commit `772bbfb` corrigió
el orden sin relajar las pruebas; la ejecución posterior
[34730284072](https://github.com/jeffersondavila/personal-blog-backend/actions/runs/34730284072)
concluyó **GREEN, 25/25 pasos, 0 skipped**. Suite final local **Windows: 1875 passed,
1 skipped** por ausencia de `time.tzset`; **CI Linux: 1876 passed**. Es un defecto
adicional del entregable, distinto de los siete H-1…H-7; detalle en el reporte §14.

**Lo que NO cierra:** **D-21** (aporta la evidencia con contenido real que pedía, pero no
elige estrategia de *rendering*), **R-018-3**, **R-021-1** y **R-018-4** siguen **abiertos**.
...
41 tareas: el avance quedó *en esa fecha* en **21/41 ≈ 51 %**. Con esta aprobación la
...
## Tarea aprobada anterior — `Task/021-CI-Infraestructura`
...
El cambio es mínimo: **tres** archivos funcionales, **una línea cada uno**,
solo el prefijo de registro. **Release y digest intactos**:
`RELEASE.2025-09-07T16-13-09Z@sha256:14cea…`. La OCI Registry API de Quay
respondió **HTTP 200 anónimo**; índice, manifiesto `amd64`, configuración,
**9** layers, **9** RootFS diff IDs, `Created`, `Entrypoint`, `Cmd` y labels
resultaron **idénticos** a la imagen ya almacenada localmente: mismo contenido
OCI, no un reempaquetado. El comparador estricto cuenta **100** aprobados,
...
`push` sobre `43c1bf2`, `completed/success`, 19 pasos verdes, run 56 s. El gate estricto
rechazó tres reclasificaciones de CVE-2026-56854 de CRITICAL a HIGH. Tras
revisión humana explícita se sustituyeron únicamente esas tres severidades
del baseline, sin cambiar imágenes, digests ni otros findings. Comparador y
pruebas intactos: **13/13 GREEN**; scan nuevo con **0** fuera del baseline.
El residual permanece aceptado temporalmente. D-021-A/B y el defecto del
comparador Resueltos; ese gate histórico comparó 116 identidades exactas,
0 fuera del baseline y residual visible. No certifica la CI del HEAD final,
que quedó bloqueada antes de ejecutar S-09.
**No aprobada**, así que el
...
- **B-021-1:** D-020-H figura como vigente y como decisión abierta. La
  limitación Windows pendiente no equivale a una decisión aprobada pendiente.
- **B-021-2:** la ficha afirma omitir hechos sobre la creación de su propio
  PR, pero su tabla inicial registra esa creación como hecho fechado válido.

**Desbloqueo autorizado y aplicado el 2026-09-10:** B-021-1 y B-021-2
**Resueltos**, con autorización explícita del usuario dentro de Task021. Se
conservaron los hechos fechados y se corrigieron únicamente las formulaciones
autorizadas en ficha/reporte Task020.1 y reporte Task020. El preflight
documental se reanudó después de la detención exigida por el prompt.
[Reporte Task021](../task-reports/TASK-021-report.md). No se creó otra maintenance.

**Medición técnica completada el 2026-09-10.** Inventario exacto, baseline de
Compose, validación de las dos familias de scripts, auditoría histórica de los
tres repositorios y cinco controles negativos locales: **todo verde**. El
historial de infra da **0 hallazgos** de secretos en 42 commits; el del
frontend, **0**; el del backend, **2 falsos positivos demostrados** en tests de
redacción, que no obligan a tocar ese repositorio.

**B-021-3 — Resuelto el 2026-09-11 por decisión explícita del usuario.** El
gate de vulnerabilidades de infraestructura **no podía quedar verde** con la
política de Task020 aplicada tal cual. Las dos imágenes que el proyecto
construye —`postgres` y `traefik`— están en **0** hallazgos, pero
`minio/minio` acumula **100** HIGH/CRITICAL con corrección aguas arriba y
`portainer-ce` **16**. **MinIO no tiene salida por actualización:** la imagen
fijada **ya es la última publicada en Docker Hub** y el release posterior de
GitHub no existe como imagen. Portainer solo bajaría a **9** con un salto de
versión mayor. Además MinIO está fijado también en el CI del backend, que
Task021 no puede modificar.

**Política adoptada:** las imágenes que **construye el proyecto** mantienen
**tolerancia cero** y su baseline está **vacío a propósito**; las de **terceros
fijadas por digest** se comparan por **identidad exacta** —ámbito, CVE,
paquete, versión instalada, severidad y `FixedVersion`— contra
[`security/vulnerability-baseline.json`](../../security/vulnerability-baseline.json),
que enumera los **100** hallazgos de MinIO y los **16** de Portainer aceptados
el 2026-09-11. La CI **falla** ante un hallazgo accionable nuevo, un digest
distinto del revisado, cualquier campo de identidad cambiado, un informe ilegible o un baseline
inválido. **No se usó** `.trivyignore`, umbral por cantidad, `|| true`,
`continue-on-error` ni exclusión de ninguna imagen: **un conteo no sirve como
allowlist**, porque podrían desaparecer cinco CVE y entrar otras cinco sin que
el número cambie. Propietarios del residual: **R-018-3** para MinIO y
**R-021-1** para Portainer. Detalle en §M, §N y §Ñ del
[reporte](../task-reports/TASK-021-report.md).

**Evidencia histórica del 2026-09-11, anterior a detectar el defecto.** El workflow **`CI Infra`** valida el
...
**34604423915**, por `push` sobre `4808d7c`, terminó en
**`success`** en **51 s** con los **15** pasos declarados en verde (**18** registrados por GitHub), **44** commits
...
cambio de severidad como hallazgo nuevo. La implementación posterior permitió rebajas de severidad. La revisión
del 2026-09-11 determinó que esa relajación era un defecto funcional:
la política aprobada exige identidad completa, también `FixedVersion`,
y un nuevo run conforme antes de acreditar S-09 infraestructura. Los
...
pasaron a **21/41 — 51 %** y **3/3** tareas de la etapa. *En esa fecha* Task022 estaba
**Pendiente**; se inició el 2026-09-12 y quedó **Lista para validación** el 2026-09-12,
sin alterar el avance.
...
| **Repositorios** | `personal-blog-infra` únicamente |
| **Rama** | `Task/020.1-Corregir-Drift-Documental-Post-Merge`, nacida de `main` (`68469dd`) |
...
`Task/020` documentó su cierre **antes** de que el usuario fusionara los pull request, así
que dejó escrito en presente lo que el paso siguiente del flujo volvió falso: que los PR
quedaban sin fusionar, que la rama remota se conservaba y que la normalización `main → dev`
estaba pendiente. Son **nueve** afirmaciones de clase **C**, ya convertidas en hechos
fechados. Es el mismo defecto estructural que [WORKFLOW §6.1](WORKFLOW.md) describe.

A eso se sumaron **ocho** contradicciones de clase **D**, dos de ellas ya detectadas y
declaradas fuera de alcance por `Task/020`: **B-020-4**, el contador vivo dentro del
registro histórico de `Task/002.1`, y **B-020-5**, los cinco encabezados «Última tarea
aprobada» simultáneos. Las otras seis son conteos internos del reporte de `Task/020` que no
cuadraban entre sí: ejecuciones de Actions, commits, documentos y alcance del barrido de
secretos.
...
## Mantenimiento aprobado previo — `Task/019.1`
...
### Cierre y normalización

*Observado el 2026-09-10 UTC.* El cierre aprobado integró la rama en `dev` con
merge `--no-ff` en los dos repositorios y creó los pull request
**`Task/020-CI-Backend → main`**, dejados para revisión manual del usuario.
Después el usuario los fusionó y eliminó las dos ramas Task remotas, y la
normalización `main → dev` quedó completada. Hechos fechados; el estado operativo
vigente se consulta en Git y GitHub ([WORKFLOW §6.1](WORKFLOW.md)).

| Repositorio | PR | `mergedAt` | Merge commit | Normalización `dev` |
| --- | --- | --- | --- | --- |
| backend | `#15` | 2026-09-10T14:42:42Z | `8055878` | `5fedcb3` |
| infra | `#36` | 2026-09-10T14:42:24Z | `68469dd` | `122c90a` |
...
criterios de salida exigen los tres repositorios.
...
Entrega controles locales de **S-01/03/04/05/07/08/10/11** y cierra **E-06** por
HTTP. Las decisiones de implementación de la [ficha §12](../tasks/TASK-018-security-hardening.md)
quedan **vigentes**. No hay ADR nuevo; **D-21** continúa **Abierta** y **ADR-009**
en **Propuesta**. Avance: **18/41 — 44 %**, ETAPA 05 **3/3 — 100 %**.

Los riesgos residuales, incluidos **R-09**, **R-12**, **R-018-1** a **R-018-4**,
**R-44** y **R-46**, conservan sus límites y propietarios. Evidencia y validaciones
del cierre en el [reporte](../task-reports/TASK-018-report.md), §R y §X.

---

## Tarea aprobada anterior — `Task/017-Observabilidad-Local`
...
## Tarea aprobada anterior — `Task/016-SEO-Accesibilidad-y-Rendimiento`
...
## Tarea aprobada anterior — `Task/015-Panel-Administrativo`
...
## Mantenimiento funcional anterior — `Task/012.1-Exponer-Auditoria-Para-Dashboard`
...
## Mantenimiento aprobado anterior — `Task/004.2`
...
## Tarea aprobada anterior — `Task/013-Sistema-de-Diseno`
...
## Tarea aprobada anterior — `Task/006-Fundacion-Frontend-React`
...
| **Avance global en su fecha** | **2 de 41 — ≈5 %**, el valor vigente el 2026-07-26. Este mantenimiento no cuenta dentro de las 41 tareas y su aprobación no alteró el avance canónico |
...
> **Corrección de `Task/020.1` (B-020-4), 2026-09-10.** Esta fila decía
> **«44 % — 18 de 41»**, un contador que no era cierto en 2026-07-26 ni después.
> El valor histórico real está **probado en el propio historial**: el commit
> `700be94`, que es el del cierre de `Task/002.1`, registraba aquí
> «Permanece en **2 de 41**, aproximadamente **5 %**», coherente con la tabla de
> avance de ese mismo commit —Total **41 · 2 aprobadas · 5 %**—. El valor
> correcto se restaura y queda **fechado**. El defecto entró el 2026-09-08 con
> el commit `83cffe4`, del cierre de `Task/018`, que actualizó el contador
...
> **Corrección de `Task/025` (D-025-1), 2026-09-14.** Estas dos tablas son un **resumen
> vigente**, no un registro fechado, y llevaban tiempo desactualizadas: declaraban
...
| 01 — Infraestructura Local | 2 | 2 | **100 %** — **completada** |
...
| Estado | Tareas |
| --- | --- |
| Pendiente | **14** — la siguiente es `Task/028`, **no iniciada** |
| En progreso | 0 |
...
| # | Riesgo | Impacto | Mitigación prevista | Tarea que lo valida | Estado |
| --- | --- | --- | --- | --- | --- |
| R-36 | **El log no tiene redacción automática de secretos.** `JsonLogFormatter` emite en `context` **todo** atributo propio del `LogRecord` y serializa las excepciones completas. La regla S-08 —«los logs no contienen contraseñas, tokens ni cadenas de conexión»— existe y se cumple hoy, pero depende de que **quien registra el evento** no pase un valor sensible: no hay ningún mecanismo que lo impida. Una excepción de driver o un `extra` descuidado pueden filtrar una credencial. Reproducido en `Task/005.6` por inspección del formateador. | Medio | **Deliberadamente NO se corrige en `Task/005.6`**: construir una política de redacción completa —lista de claves sensibles, patrones de token y URL, redacción en mensaje, contexto y traza— es trabajo de observabilidad y endurecimiento, no de cierre de fundaciones. Mitigación vigente: `database_url` está excluida de `repr` y solo se expone por `database_url_safe`, verificado en el contenedor real (`blog_local:***@`); `ConfigurationError` nombra campos, nunca valores. | `Task/017` (observabilidad, correlation ID y política de log) y `Task/018` (endurecimiento de seguridad) | **Cerrado en el plano local** por `Task/017`, **Aprobada** el 2026-09-06. El mecanismo existe y está probado: redacción por **nombre de campo** y por **forma del valor**, aplicada al mensaje, al contexto y a la **cadena de excepciones**, en `json` y en `text`, idempotente y sin destruir el diagnóstico —la DSN conserva esquema, anfitrión, puerto y base; la excepción conserva su tipo—. Las pruebas siembran un señuelo y exigen que no aparezca en la salida. La causa que abrió el riesgo —*«depende de que quien registra el evento no pase un valor sensible»*— **deja de sostenerse en el plano local**. **Endurecimiento local completado** con la aprobación de `Task/018` del 2026-09-08: cabeceras completas, URLs autenticadas y campos con guion, verificados con señuelos. **Sigue abierto** únicamente el plano de telemetría **exportada** (**O-09**, `Task/029`, `Task/040`), según la tabla de efecto de `Task/018` | **Abierto** — residuo de telemetría exportada |

### Efecto de `Task/018` sobre los riesgos vivos — 2026-09-07

> **`Task/018` aprobada el 2026-09-08.** Se conserva la medición del 2026-09-07
> de cada riesgo y su residuo declarado; la aprobación no equivale a cerrar
> los riesgos que esta tabla mantiene abiertos.

| # | Baseline antes de `Task/018` | Qué hizo `Task/018` | Evidencia | Estado final | Propietario del residuo |
| --- | --- | --- | --- | --- | --- |
| **R-09** | Portainer con socket de Docker: capacidad administrativa sobre el host. `:ro` protege el archivo, no la API | **Nada que cambie sus capacidades, deliberadamente.** Se le retiraron capacidades del kernel y se le puso la raíz en solo lectura, lo que **no** toca el socket. Introducir un *socket proxy* sería un cambio de capacidades y exige decisión propia | Contenedor con `cap_drop: [ALL]`, `no-new-privileges`, `read_only`; mismo volumen `personal-blog-local_portainer_data`; autenticación administrativa **HTTP 200** contra `2.39.7` | **Abierto, sin agravar.** Mitigación intacta: solo local, `127.0.0.1`, autenticación propia | Decisión pendiente; `Task/025` lo reevalúa junto a **R-22** |
| **R-10** | Etiquetas fijadas que envejecen | Auditoría **puntual y fechada** de las siete imágenes y actualización de las que tenían parche; todas fijadas además **por digest** | Escaneo del 2026-09-07 de cada imagen final; recuento por severidad en el reporte §F | **Abierto.** Una auditoría puntual caduca; la garantía duradera es CI | `Task/019`–`Task/021` (**S-09**) |
| **R-12** | Respaldos locales sin cifrar, con *hashes* de Portainer dentro | **No los cifra, y no finge hacerlo.** Verificó integridad de los cinco conjuntos y probó la **restauración real** del conjunto posterior al endurecimiento | 5 conjuntos íntegros; restauración aislada con PostgreSQL, MinIO (64 objetos, SHA-256 y `Content-Type`) y Portainer **CORRECTAS** | **Abierto.** Siguen sin cifrar, ignorados por Git y limitados a la máquina | **D-17** / `Task/029` |
| **R-15** | Imagen base del backend envejecida | Actualizada a `python:3.12.14-slim` fijada por digest, y `pip` retirado de la imagen final | Escaneo del 2026-09-07: los `CRITICAL` y `HIGH` restantes son paquetes de Debian **sin versión corregida publicada** | **Abierto.** Sin parche aguas arriba no hay acción posible hoy | `Task/020` (CI) para detectarlo cuando lo haya |
| **R-36** | Redacción del log cerrada en el plano local por `Task/017`; quedaba el endurecimiento | Cabeceras `Authorization`, `Cookie` y `Set-Cookie` completas, contraseña **y usuario** de una URL autenticada, y claves con guion o guion bajo | Señuelos en mensaje, contexto y **cadena de excepciones**, en `json` y `text`; idempotencia comprobada; el diagnóstico y el `request_id` sobreviven | **Cerrado en el plano local.** Sigue abierto solo para la telemetría **exportada**, que es **O-09** | `Task/029`, `Task/040` |
| **R-43** | Bloqueo temporal puede dejar fuera al único administrador | **No se reescribió nada.** Se auditó y se registró la evidencia existente | 14 pruebas unitarias y 11 de integración: umbral, el bloqueo **no se alarga** con fallos durante el bloqueo, vencimiento, concurrencia y respuesta **indistinguible** de credenciales inválidas | **Abierto.** Es una propiedad asumida del diseño, no un defecto | Recuperación operativa; `Task/029` |
| **R-44** | Tablas de estado de autenticación que crecen sin purga | **No se introdujo ningún proceso residente**, por decisión: contradiría el destino Lambda | Búsqueda dirigida: no hay *scheduler*, ni tarea de fondo, ni `cron` en el backend | **Abierto y diferido a propósito** | `Task/029` (operación) |
| **R-46** | Confianza en proxies sin fijar para ningún despliegue | El Compose local fija `BLOG_TRUSTED_PROXY_HOP_COUNT=0`, el valor **fail-closed** | Pruebas de `Task/011` en ambos sentidos: cabecera falsificada **no** evade el límite; con proxy declarado **sí** se atiende | **Abierto.** Lo que falta es decidir los saltos de cada despliegue real | `Task/033` |

**Riesgos nuevos que `Task/018` descubre**, y que **no existían antes de la tarea**:

| # | Riesgo | Impacto | Mitigación aplicada | Estado |
| --- | --- | --- | --- | --- |
...
> **Abierto y vigente** desde el 2026-09-11. **No** lo cubre **R-09**, que trata
> la capacidad administrativa del socket de Docker, ni **R-12**, que trata los
> respaldos sin cifrar. Es un riesgo distinto: las vulnerabilidades de la propia
> imagen.

| # | Riesgo | Impacto | Mitigación aplicada | Estado |
| --- | --- | --- | --- | --- |
| **R-021-1** | **La imagen de Portainer conserva vulnerabilidades accionables.** Medido el 2026-09-11 sobre `2.39.7`: **1 CRITICAL** y **17 HIGH**, de los que **16** tienen corrección publicada aguas arriba | Medio | Servicio **solo local**, publicado en loopback, con autenticación propia y **fuera de la arquitectura de producción**. El residuo **no se oculta**: `CI Infra` lo compara contra un **baseline exacto** ligado al digest y **falla ante cualquier hallazgo nuevo**. La mejor versión disponible, `2.45.0`, **reduce el residuo de 16 a 9 pero no lo elimina**, y saltar de la línea `2.39` a la `2.45` afecta a un componente con **volumen persistente**: no se actualiza dentro de `Task/021` | **Abierto**, con baseline vivo en [`security/vulnerability-baseline.json`](../../security/vulnerability-baseline.json) |
...
| `Task/027-Configurar-Cuentas-y-Presupuestos` | 09 | infra | **Aprobada** (2026-09-17) |
...
- Las dos ramas `Task/020-CI-Backend` —backend e infra— nacieron **desde `main`**, con
  `HEAD == main` verificado inmediatamente después de crearlas: `8762bbad…` (backend) y
  `c5b16070…` (infra).
- El backend llevaba **4 commits** propios sobre `main` —`557ca7e`, `f2b3d85`, `81ce14c` y
  `22af3f1`—, publicados antes de la aprobación bajo la **autorización excepcional de
  bootstrap** concedida solo a ese repositorio, para poder validar CI real. Infra llegó al
  cierre con **0 commits** propios y nada en *staging*; sus siete documentos se commitearon
  en el propio cierre como **`6bc80e8`**.
...
- El cierre integró cada rama en `dev` con merge `--no-ff`, publicó `dev` y la rama Task, y
  abrió el pull request **`Task/020-CI-Backend → main`** en cada repositorio: **`#15`** en
  backend y **`#36`** en infra. **Ningún PR usó `dev` como *head*** y **ninguno lo fusionó
...
**Observado el 2026-09-10 UTC**, tras la fusión manual de los dos PR por el usuario y la
normalización posterior:

- **PR `#15`** (backend) fusionado, `mergedAt = 2026-09-10T14:42:42Z`, merge commit
  **`8055878`**. **PR `#36`** (infra) fusionado, `mergedAt = 2026-09-10T14:42:24Z`, merge
  commit **`68469dd`**. El usuario eliminó las dos ramas Task remotas.
