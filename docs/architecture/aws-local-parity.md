...
| **Quién lo valida de verdad** | Task/025 creación; Task/028 federación sin políticas de recursos; Task/032 autorización del rol Lambda; Task/038 y Task/039 permisos mínimos de despliegue; Task/040 integral |
...
> aquí en lugar de heredarse. Una diferencia concreta quedó **sin re-evaluar** y se anota en
> su fila: el CHANGELOG de `2.0.0` menciona un cambio en la evaluación de alarmas de
...
## Addendum Task/028 — federación fuera del emulador

Checkpoint del 2026-09-22: root IAM independiente y pruebas offline, **federación
real pendiente**. No cambia ninguna celda a Validada en AWS. Task/028 valida la
identidad GitHub → STS sin políticas de recursos, no el rol de aplicación Task/032.
Task/038 y Task/039 definen permisos de despliegue; Task/040 valida integralmente.

D-06 está Resuelta desde Task/025. Bucket pendiente; EX-028-C7 permite solo estado
bootstrap OIDC local privado hasta materialización/migración por Task/030, antes
del primer apply de aplicación. ETAPA 10 incorpora la excepción explícita del
bootstrap S3 dedicado sin duplicar los módulos de aplicación Task/025.
[Runbook](../runbooks/github-oidc-bootstrap.md).
